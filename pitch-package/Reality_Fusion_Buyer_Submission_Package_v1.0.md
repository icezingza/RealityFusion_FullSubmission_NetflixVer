# REALITY FUSION — BUYER SUBMISSION PACKAGE

Version: 1.0
Status: INTERNAL ASSEMBLY COMPLETE — EXTERNAL GATES PENDING

## Purpose

This index identifies the current-canon files to use when preparing a tailored approach to a producer, studio, broadcaster, streamer, or investor. It is an internal assembly guide, not a representation that the project is production-ready, legally cleared, or accepted by any recipient.

## Core buyer-facing materials

| Material | Current file |
|---|---|
| One-page pitch | `pitch-package/en/Reality_Fusion_One_Page_Pitch_EN_v2.0.md` |
| Logline and short synopsis | `pitch-package/en/Reality_Fusion_Logline_and_Series_Synopsis_EN_v2.0.md` |
| Episode guide | `pitch-package/en/Reality_Fusion_Episode_Guide_EN_v2.0.md` |
| Creator statement | `pitch-package/en/Reality_Fusion_Creator_Statement_EN_v2.0.md` |
| Pitch-deck copy | `pitch-package/en/Reality_Fusion_Pitch_Deck_Copy_EN_v2.0.md` |
| Current screenplay sample | `deliverables/current-screenplays/pdf/Reality_Fusion_EP1_Master_Draft_v2.1.pdf` |
| Full current screenplay set | `deliverables/current-screenplays/` |
| Current canon and continuity source | `CANON.md` and `canon/` |

## Verified package facts

- The active screenplay set is EP1 v2.1, EP2 v2.1, EP3 v2.2, EP4 v2.1, and EP5 v2.1.
- The full formatting package contains five DOCX files, five PDF files, a README, and a ZIP archive.
- The PDFs were parsed successfully. Their formatting page counts are EP1 18, EP2 18, EP3 33, EP4 19, and EP5 21.
- Formatting page counts are not runtime estimates.

## Per-recipient assembly

1. Check the recipient's current submission policy before sending any material.
2. Send only the files they request. Default starting set: one-page pitch, logline/synopsis, creator statement, episode guide, and the EP1 PDF only when a script sample is requested.
3. Use the pitch-deck copy only after visual assets have been selected and cleared for use.
4. Record recipient, date, submitted files, requested format, and response in a private submission log. Do not place confidential correspondence in this repository.
5. Do not represent any external review, rights clearance, budget, production schedule, runtime, attachment, financing, or recipient interest as complete unless separately documented.

## Required gates before an external sales approach

See `production/Reality_Fusion_Pitch_Readiness_Gate_v1.0.md`. The package may be assembled now, but a creator decision is required for every open gate.
